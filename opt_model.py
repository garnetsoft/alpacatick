# ### q code
# / get predicted close using options 
# predClose:raze get_pred_close2[;`AAPL`UBER`TSLA`LK`XOM`MSFT`IBM] each exec distinct lastUpdated from (select by lastUpdated from putCallDaily where lastUpdated>.z.D-100)
# select from predClose where symbol=`LK
# 
# get_pred_close2:{[d;tickers]
#  optDate:d;
#  optData: raze{
#   / d:.z.D-1; x:`AAPL;
#   f:":/data/options/",(string x),"_put_call_ratio",ssr[(string y);".";""],".csv";
#   if [()~key `$f;:()];
#   data:("SSFDDIIFFFF";enlist",") 0:`$f;
#   :data
#   }[;optDate] each tickers;
#  
#  predDaily: raze {
#   show " " sv (string x; string y; string (count z));
#   s2:x;optDate2:y;
#   / expDate2:optDate2+1;
#   t2:select from z where symbol=s2, lastUpdated=optDate2;
#   / look at the nearest expirationDate 
#   expDate2:min exec distinct expirationDate from t2;
# 
#   show count t2;
#   if [(count t2) <2;:()];
#   / show "xxxx computing payout on each strike: ",(string x);
# 
#  1#`payout xasc raze {
#   px:x; y2:y;
#   s2:y[0];
#   expDate2:y[1];
#   exptbl:select from y2[2] where symbol=y2[0], expirationDate=y2[1];
#   / show "xxxx me: ",string (count me);
# 
#   update optExpDate:expDate2, predClose:px from select last lastUpdated, last symbol, payout:sum abs Wow from 
#   (
#    select Wow:floor sum dollar*volume, count i, sum volume, sum dollarValue, avg spot, minK:min strikePrice, maxK:max strikePrice by lastUpdated, symbol, side, optype from 
#    update dollar:strikePrice-spot from update optype:?[side=`call;?[strikePrice<spot;`ITM;`DEAD];?[strikePrice>spot;`ITM;`DEAD]] from update spot:x from exptbl
#   ) where optype=`ITM
# 
#   }[;(s2;expDate2;t2)] each exec distinct strikePrice from (select from t2)
#  }[;optDate;optData] each tickers }
#  / }[;optDate;optData] each (exec distinct symbol from select from putCallDaily where lastUpdated=optDate)
#  
#### 
...
class Option(Instrument):
    ...
    @staticmethod
    def imply_volatility(premium, max_vol=1.99, *args, **kwargs):
    ...
    def black_scholes(self):
        ...
        assert (self.yield_ is None) or is_number(self.yield_),             "Black-Scholes does not support discrete dividends"
        ...
        if self.opt_type == OptionType.CALL:
            ...
            theta = -yield_disc * (self.spot0 * d1_pdf * self.vol) / (2 * sqrt_mat) -                 self.riskless_rate * self.strike * riskless_disc * d2_cdf +                 self.yield_ * self.spot0 * yield_disc * d1_cdf
            ...
            theta = -yield_disc * (self.spot0 * d1_pdf * self.vol) / (2 * sqrt_mat) +                 self.riskless_rate * self.strike * riskless_disc * neg_d2_cdf -                 self.yield_ * self.spot0 * yield_disc * neg_d1_cdf
        ...
        def node_value(n, num_ups, num_downs):
            ...
            if value_cache_key not in val_cache:
                ...
                if n >= val_num_steps:
                    ...
                    fv = prob_up * node_value(n + 1, num_ups + 1, num_downs) + (1 - prob_up) * node_value(n + 1, num_ups, num_downs + 1)
...
def is_number(x):
    return (x is not None) and            (isinstance(x, int) or isinstance(x, float) or isinstance(x, numbers.Integral))
